...
3. Apps Script 또는 GitHub Pages 배포 경로·명령 변경 시 ref.md와 handoff.md를 갱신한다.
4. 새 문서를 추가하거나 문서 위치가 바뀌면 ref.md의 문서 목록과 이 파일의 필수 확인 파일을 갱신한다.
5. README.md는 아직 미작성이다. 사용자용 실행·배포 안내가 필요해져 새로 만들면 ref.md와 이 파일에 추가한다.
6. 작업 완료 후 실행한 검증과 미실행 사유를 handoff.md에 기록한다.
```
